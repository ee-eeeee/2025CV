...
if __name__ == '__main__':
    ...
    parser.add_argument('-u', '--use_camera', action='store_true')

    # 根据视频尺寸，填充一个polygon，供撞线计算使用
    mask_image_temp = np.zeros((1080, 1920), dtype=np.uint8)
    # 初始化2个撞线polygon
    if parser.parse_args().use_camera:
        list_pts_blue = [[905, 0], [955, 0], [955, 1080], [905, 1080]]
    else:
    ...
    mask_image_temp = np.zeros((1080, 1920), dtype=np.uint8)
    if parser.parse_args().use_camera:
        list_pts_yellow = [[965, 0], [1015, 0], [1015, 1080], [965, 1080]]
    else:
    ...
    if parser.parse_args().use_camera:
        ...
        capture = cv2.VideoCapture('./video/test.avi')
    ...
    while True:
        ...
        # 保存到文件夹
        cv2.imwrite('./output/' + str(int(cv2.getTickCount())) + '.jpg', output_image_frame)
        cv2.waitKey(1)
        pass
    pass
    ...
    for file in os.listdir('./output'):
        if file.endswith('.jpg'):
            ...
            if 'out.avi' not in os.listdir('./output'):
                height, width, layers = img.shape
                fourcc = cv2.VideoWriter_fourcc(*'XVID')
                out = cv2.VideoWriter('./output/out.avi', fourcc, 20.0, (width, height))
            out.write(img)
